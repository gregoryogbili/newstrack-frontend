          ...
          {["6h", "24h", "72h"].map((w) => (
            <button
              ...
              style={{
                padding: "6px 12px",
                borderRadius: 6,
                border: windowParam === w ? "1px solid #b80000" : "1px solid #333",
        ...
        <div style={grid}>
          ...
          <div style={rowBand} className="rowBand">
            ...
            <div style={{ ...panel, gridColumn: "span 3" }}>
              <div
                ...
                onMouseEnter={() => setTooltip("Measures acceleration in narrative volume relative to baseline clusters.")}
                onMouseLeave={() => setTooltip(null)}
              >
                Signal Velocity Index ⓘ
              </div>
              <div style={metric}>{overview ? overview.velocityIndex : "--"}</div>
              {overview?.delta?.velocity !== null && overview?.delta?.velocity !== undefined && (
                <div style={{ fontSize: 12, fontWeight: 700, marginTop: 4, color: overview.delta.velocity > 0 ? "#ef4444" : "#22c55e" }}>
                  {overview.delta.velocity > 0 ? "▲" : "▼"} {Math.abs(overview.delta.velocity)}% vs yesterday
                </div>
              )}
              <div style={metricLabel}>Change in global narrative activity</div>
              <div style={{ ...metricLabel, fontSize: 11, opacity: 0.6 }}>(0–100 scale, {windowParam} window)</div>
            </div>
            ...
            <div style={{ ...panel, gridColumn: "span 3" }}>
              <div
                style={{ ...panelTitle, cursor: "help" }}
                onMouseEnter={() => setTooltip("Blended pressure score combining volume, acceleration density, and geographic dispersion.")}
                onMouseLeave={() => setTooltip(null)}
              >
                Narrative Pressure Index ⓘ
              </div>
              ...
              <div
                style={{ fontSize: 11, marginTop: 6, cursor: "pointer", color: "#e8e4df" }}
              ...
              {showNPIBreakdown &&
                ...
                (() => {
                  const total = overview.narrativeBreakdown.reduce((sum, n) => sum + (n.score || 0), 0);
                  return (
                    <div style={{ marginTop: 12, padding: 14, borderRadius: 10, background: "#000", border: "1px solid rgba(184,0,0,0.3)" }}>
                      <div style={{ fontSize: 11, fontWeight: 800, marginBottom: 10, color: "#e8e4df", letterSpacing: 1 }}>
                        Narrative Drivers
                      </div>
                      {overview.narrativeBreakdown.map((n, i) => {
                        ...
                        return (
                          <div key={i} style={{ marginBottom: 10 }}>
                            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, letterSpacing: "0.3px", marginBottom: 4, color: "#a09e9b", fontWeight: 500 }}>
                              <span>{n.label}</span>
                              <span style={{ color: "#e8e4df", fontWeight: 600 }}>{Math.round(n.score)} ({Math.round(percent)}%)</span>
                            </div>
                            <div style={{ height: 6, background: "#1a0000", borderRadius: 4, overflow: "hidden" }}>
                              <div style={{ width: `${percent}%`, height: "100%", background: "#b80000", transition: "width 0.4s ease" }} />
                            </div>
                          </div>
                        );
                      })}
            ...
            <div style={{ ...panel, gridColumn: "span 3" }}>
              <div
                style={{ ...panelTitle, cursor: "help" }}
                onMouseEnter={() => setTooltip("Composite risk derived from velocity, acceleration density, and cluster instability.")}
                onMouseLeave={() => setTooltip(null)}
              >
                Economic Risk Pulse ⓘ
              </div>
              {!overview ? (
                ...
                <div>
                  ...
                  {overview?.delta?.econ !== null && overview?.delta?.econ !== undefined && (
                    <div style={{ fontSize: 12, fontWeight: 700, marginTop: 4, color: overview.delta.econ > 0 ? "#ef4444" : "#22c55e" }}>
                      {overview.delta.econ > 0 ? "▲" : "▼"} {Math.abs(overview.delta.econ)}% vs yesterday
                    </div>
                  )}
                  <div style={{ marginTop: 6, fontSize: 12, fontWeight: 600, color: overview.economicRisk >= 70 ? "#ef4444" : overview.economicRisk >= 40 ? "#f59e0b" : "#22c55e" }}>
                    {overview.economicRisk >= 70 ? "HIGH INSTABILITY" : overview.economicRisk >= 40 ? "ELEVATED RISK" : "LOW PRESSURE"}
                  </div>
                </div>
              )}
            ...
            <div style={{ ...panel, gridColumn: "span 3" }}>
              <div
                style={{ ...panelTitle, cursor: "help" }}
                onMouseEnter={() => setTooltip("Composite score combining narrative velocity, pressure, economic risk and acceleration density.")}
                onMouseLeave={() => setTooltip(null)}
              >
              ...
              <div style={{
                fontSize: 32,
                fontWeight: 300,
                color: !overview ? "#e8e4df" : overview.globalTensionIndex >= 70 ? "#ef4444" : overview.globalTensionIndex >= 40 ? "#f59e0b" : "#22c55e",
                lineHeight: 1,
                marginBottom: 8,
              }}>
              ...
              <div style={{
                fontSize: 13,
                fontWeight: 700,
                color: !overview ? "#a09e9b" : overview.globalTensionIndex >= 70 ? "#ef4444" : overview.globalTensionIndex >= 40 ? "#f59e0b" : "#22c55e",
                marginBottom: 8,
              }}>
                {!overview ? "" : overview.globalTensionIndex >= 70 ? "CRITICAL" : overview.globalTensionIndex >= 50 ? "ELEVATED" : overview.globalTensionIndex >= 30 ? "MODERATE" : "STABLE"}
              </div>
              ...
              <div style={{ marginTop: 16 }}>
                {[
                  ...
                  <div key={item.label} style={{ marginBottom: 8 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, marginBottom: 3, color: "#a09e9b" }}>
                      <span>{item.label}</span>
                      <span>{item.value ?? "--"}</span>
                    </div>
                    <div style={{ height: 4, background: "#1a0000", borderRadius: 4, overflow: "hidden" }}>
                      <div style={{ width: `${item.value ?? 0}%`, height: "100%", background: "linear-gradient(90deg, #b80000, #ef4444)" }} />
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div style={rowBand} className="rowBand">
            <div style={{ ...panel, gridColumn: "span 6" }}>
              <div
                style={{ ...panelTitle, cursor: "help" }}
                onMouseEnter={() => setTooltip("Region-weighted narrative intensity adjusted for acceleration and divergence.")}
                onMouseLeave={() => setTooltip(null)}
              >
                  ...
                  <div
                    ...
                    onClick={() => {
                      fetch(`${API}/signals/region/${r.region}`)
                        .then((res) => res.json())
                        .then((data) => { setSelectedRegion(data); setPanelOpen(true); });
                    }}
                  >
                    <strong>{r.region}</strong>
                    <div style={{ fontSize: 12, opacity: 0.85, marginTop: 6 }}>
                      Narrative Activity: {r.narrativePressure > 70 ? "High" : r.narrativePressure > 40 ? "Elevated" : "Calm"}
                      {" | "}Political Tension: {r.riskPressure > 70 ? "High" : r.riskPressure > 40 ? "Elevated" : "Low"}
                      {" | "}Strategic Movement: {r.strategicPressure > 70 ? "Active" : r.strategicPressure > 40 ? "Developing" : "Minimal"}
                      {" | "}Market Sensitivity: {r.marketPressure > 120 ? "High" : r.marketPressure > 80 ? "Elevated" : "Stable"}
                    </div>
                  </div>
                ))
              )}
            ...
            <div style={{ ...panel, gridColumn: "span 6" }}>
              ...
              {!overview?.regionalSpread ? (
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
                  <div>
                    <div style={{ fontSize: 12, opacity: 0.6, marginBottom: 8 }}>Top 10 by Volume</div>
                    {overview.regionalSpread.slice(0, 10).map((r) => (
                      <div key={r.region} style={listItem}>• {r.region} <span style={{ opacity: 0.65 }}>({r.count})</span></div>
                    ))}
                  </div>
                  <div>
                    <div style={{ fontSize: 12, opacity: 0.6, marginBottom: 8 }}>Top 10 by Acceleration</div>
                    {overview.geopoliticalPressure?.slice(0, 10).map((r) => (
                      <div key={r.region} style={listItem}>• {r.region} <span style={{ opacity: 0.65 }}>({r.narrativePressure})</span></div>
                    ))}
                  </div>
                </div>
              )}
          ...
          <div style={rowBand} className="rowBand">
              ...
              {!clusters.length ? (
                ...
                <>
                  <div style={{ marginBottom: 12 }}>
                    <div style={{ fontSize: 11, fontWeight: 700, color: "#4ade80", marginBottom: 6, letterSpacing: 0.5 }}>↑ RISING</div>
                    {clusters.filter((c) => c.momentum === "rising" || c.momentum === "accelerating").slice(0, 3).map((c, i) => (
                      <div key={i} style={{ fontSize: 12, color: "#e8e4df", marginBottom: 4, opacity: 0.9 }}>• {c.title}</div>
                    ))}
                    {clusters.filter((c) => c.momentum === "rising" || c.momentum === "accelerating").length === 0 && (
                      <div style={{ fontSize: 12, opacity: 0.5 }}>None detected</div>
                    )}
                  </div>
                  <div>
                    <div style={{ fontSize: 11, fontWeight: 700, color: "#ef4444", marginBottom: 6, letterSpacing: 0.5 }}>↓ FALLING</div>
                    {clusters.filter((c) => c.momentum === "falling" || c.momentum === "cooling").slice(0, 3).map((c, i) => (
                      <div key={i} style={{ fontSize: 12, color: "#e8e4df", marginBottom: 4, opacity: 0.9 }}>• {c.title}</div>
                    ))}
                    {clusters.filter((c) => c.momentum === "falling" || c.momentum === "cooling").length === 0 && (
                      <div style={{ fontSize: 12, opacity: 0.5 }}>None detected</div>
                    )}
                  </div>
                </>
              ...
              {!overview?.narrativeWatch?.length ? (
                ...
                overview.narrativeWatch.map((w, i) => (
                  <div key={i} style={{ marginBottom: 12 }}>
                    <div style={{ fontSize: 13, fontWeight: 600, color: "#e8e4df", marginBottom: 4 }}>{w.title}</div>
                    <div style={{ fontSize: 11, color: "#e8e4df", marginBottom: 2 }}>Dominant: {w.dominantBloc} ({w.dominantCount} articles)</div>
                    <div style={{ fontSize: 11, color: "#a09e9b" }}>Absent from: {w.missingBlocs.join(", ")}</div>
                    <div style={{ fontSize: 10, opacity: 0.5, marginTop: 2 }}>Suppression score: {w.suppressionScore}/100</div>
                  </div>
                ))
              )}
            </div>
            ...
            <div style={{ ...panel, gridColumn: "span 3" }}>
              ...
              <div style={{ fontSize: 11, opacity: 0.6, marginBottom: 10 }}>Split Narratives</div>
              {!clusters.length ? (
                <div style={listItem}>Loading clusters...</div>
              ) : (
                clusters
                  .filter((c) => (c.divergenceScore ?? 0) >= 30 && (c.sourceCount ?? c.sources ?? 0) >= 2)
                  ...
                  .map((c) => (
                    <div key={c.slug} style={listItem}>
                      ...
                      <div style={{ fontSize: 12, opacity: 0.7, marginTop: 4 }}>
                        Divergence: <strong>{c.divergenceScore ?? 0}/100</strong> • Sources: {c.sourceCount ?? c.sources ?? "--"}
                      </div>
                    </div>
                  ))
              )}
                  ...
                  return (
                    <div key={i} style={{ marginBottom: 8 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, marginBottom: 3 }}>
                        ...
                        <span style={{ color: "#e8e4df", fontWeight: 600 }}>{s.count}</span>
                      </div>
                      <div style={{ height: 4, background: "#1a0000", borderRadius: 4, overflow: "hidden" }}>
                        <div style={{ width: `${pct}%`, height: "100%", background: "linear-gradient(90deg, #b80000, #ef4444)" }} />
                      </div>
                    </div>
                  );
                })
          ...
          <div style={rowBand}>
            <div style={{ ...panel, gridColumn: "span 6" }}>
              <div style={panelTitle}>Geostrategic Narrative Intensity Ranking</div>
              {!overview?.strategicIntensityRanking ? (
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, fontSize: 13 }}>
                  {overview.strategicIntensityRanking.map((c, i) => {
                    const maxScore = overview.strategicIntensityRanking[0]?.score || 1;
                    const widthPercent = (c.score / maxScore) * 100;
                    return (
                      <div key={c.country} style={{ padding: "6px 0" }}>
                        <div style={{ fontWeight: 600 }}>{i + 1}. {c.country}</div>
                        <div style={{ height: 6, background: "#1a0000", borderRadius: 4, marginTop: 4, overflow: "hidden" }}>
                          <div style={{ width: `${widthPercent}%`, height: "100%", background: "#ef4444" }} />
                        </div>
                        <div style={{ fontSize: 11, opacity: 0.6 }}>Score: {c.score}</div>
                      </div>
                    );
                  })}
                </div>
            ...
            <div style={{ ...panel, gridColumn: "span 6" }}>
              <div
                style={{ ...panelTitle, cursor: "help" }}
                onMouseEnter={() => setTooltip("Identifies stories dominated by few sources relative to article volume — a signal of echo chamber or coordinated narrative push.")}
                onMouseLeave={() => setTooltip(null)}
              >
                Narrative Concentration Index ⓘ
              </div>
              <div style={metricLabel}>Echo chamber detector — high ratio = few sources, many articles</div>
              <div style={{ marginTop: 12 }}>
                {concentrationClusters.length === 0 ? (
                  ...
                  concentrationClusters.map((c, i) => {
                    ...
                    const risk = ratio >= 10 ? { label: "HIGH", color: "#ef4444" } : ratio >= 5 ? { label: "ELEVATED", color: "#f59e0b" } : { label: "NORMAL", color: "#22c55e" };
                    return (
                      <div key={c.slug || i} style={{ marginBottom: 14, paddingBottom: 14, borderBottom: "1px solid rgba(184,0,0,0.15)" }}>
                        <div style={{ fontSize: 13, fontWeight: 600, color: "#e8e4df", marginBottom: 4 }}>{c.title}</div>
                        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                          <div style={{ fontSize: 11, color: "#a09e9b" }}>
                            {c.articleCount ?? c.articles ?? "?"} articles · {sources} source{sources !== 1 ? "s" : ""}
                          </div>
                          <div style={{ fontSize: 10, fontWeight: 800, color: risk.color, letterSpacing: 0.5 }}>
                            {risk.label} · {ratio}x ratio
                          </div>
                        </div>
                        <div style={{ height: 4, background: "#1a0000", borderRadius: 4, marginTop: 6, overflow: "hidden" }}>
                          <div style={{ width: `${Math.min(100, ratio * 5)}%`, height: "100%", background: risk.color }} />
                        </div>
                      </div>
                    );
                  })
                  ...
                  >
                    <div
                      style={{
                        ...clusterCard,
                        borderLeft:
                          cluster.momentum === "accelerating" ? "3px solid #ff2b2b"
                          : cluster.momentum === "rising" ? "3px solid #4ade80"
                          : cluster.momentum === "falling" ? "3px solid #ef4444"
                          : "3px solid #e8e4df",
                      }}
                    ...
                    >
                      ...
                      <div style={clusterMeta}>{cluster.sources} Sources • {cluster.articles} Articles</div>
                      <div style={{ fontSize: 10, marginTop: 6, opacity: 0.65 }}>
                        Velocity: {cluster.recent} | Baseline: {cluster.previous}
                      </div>
                      ...
                      {cluster.hasDivergence && (
                        <div style={{ marginTop: 6, fontSize: 11, fontWeight: 700, color: "#f59e0b", letterSpacing: 0.4 }}>
